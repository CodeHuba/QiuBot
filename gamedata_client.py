    ...
    def replace_ph(m: re.Match) -> str:
        ...
        if prefix == "ability":
            ...
            if sub == "targets":
                    ...
                    # 尝试从 tier_attrs 取 SlowTargets / FreezeTargets / HasteTargets 等
                    atype = action.get("$type", "")
                    if "Slow" in atype:
                        v = tier_attrs.get("SlowTargets")
                    elif "Freeze" in atype:
                        v = tier_attrs.get("FreezeTargets")
                    elif "Haste" in atype:
                        v = tier_attrs.get("HasteTargets")
                    else:
                        v = None
                    return fmt_val(v) if v is not None else "1"
